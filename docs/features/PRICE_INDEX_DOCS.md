...
2. [Cara Kerja Sistem Harga AVG](#-cara-kerja-sistem-harga-avg)
3. [Struktur Project](#-struktur-project)
4. [Modul Aplikasi](#-modul-aplikasi)
5. [Route (Web & API)](#-route-web--api)
6. [Perintah Berguna (Artisan)](#-perintah-berguna-artisan)
7. [Database Schema](#-database-schema)
8. [Dokumentasi Lanjutan & Alur](#-dokumentasi-lanjutan--alur)
    ...
    - [Incremental Moving Average](#incremental-moving-average)
...
2.  **Auto-Calculated Price Index:**
    ...
    - **Dual-Mode AVG System:**
        - **AVG Otomatis (`avg_price`)**: Dihitung real-time dari transaksi yang masuk dalam range [Min, Max] menggunakan Incremental Moving Average.
        - **AVG Manual (`avg_price_manual`)**: Override manual oleh Owner/Atasan untuk situasi khusus (kontrak, negosiasi supplier, dll).
        - **Prioritas**: Jika AVG Manual ada → gunakan manual, jika tidak → gunakan otomatis.
...
## 🎯 Cara Kerja Sistem Harga AVG

Sistem Price Index menggunakan **Dual-Mode AVG System** yang memisahkan perhitungan otomatis dengan override manual untuk memberikan fleksibilitas maksimal kepada management.

### 📊 Arsitektur Dual-Mode

```
┌─────────────────────────────────────────────────────────────┐
│           TRANSAKSI APPROVED (Harga dalam Range)            │
│                    [Min ≤ Harga ≤ Max]                      │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
         ...
         │   Incremental Moving Avg    │     dari transaksi
         │   Update setiap approval    │     yang masuk range
         └─────────────────────────────┘
                       │
                       ▼
         ┌─────────────────────────────┐
         │  avg_price_manual (MANUAL)  │ ◄── Diset manual oleh
         │  Override oleh Owner/Atasan │     Owner/Atasan
         │  Tidak terpengaruh transaksi│     (opsional)
         └─────────────────────────────┘
                       │
                       ▼
              ┌────────────────┐
            ...
            └─────────┘ └─────────┘
                   ...
                   └───┬───┘
                       ▼
              ┌────────────────┐
              │ EFFECTIVE AVG  │ ◄── Harga yang ditampilkan
              │ (Yang Dipakai) │     ke user & sistem
              └────────────────┘
```
...
```
Transaksi pertama approved: Harga = Rp 50,000
┌─────────────────────────────────────────┐
│ avg_price        = Rp 50,000 (auto)    │
...
```
Existing: avg_price = Rp 50,000 (dari 10 transaksi)
Transaksi baru approved: Harga = Rp 52,000 (dalam range [45k - 60k])
...
```
Owner ingin set harga referensi berdasarkan kontrak supplier:
Set avg_price_manual = Rp 55,000

┌─────────────────────────────────────────┐
│ avg_price        = Rp 50,182 (tetap)   │
│ avg_price_manual = Rp 55,000 ✅ Manual │
...
```
Transaksi baru approved: Harga = Rp 51,000

Formula Incremental Moving Average (tetap jalan):
new_avg = ((50,182 × 11) + 51,000) / 12 = Rp 50,265

┌─────────────────────────────────────────┐
│ avg_price        = Rp 50,265 ✅ Update │
│ avg_price_manual = Rp 55,000 (tetap)   │
│ Effective AVG    = Rp 55,000 (manual)  │
└─────────────────────────────────────────┘

💡 Catatan: avg_price tetap dihitung otomatis di background
   untuk tracking harga pasar real-time, meskipun tidak digunakan.
```
...
Sistem kembali menggunakan harga otomatis.
```

### 🎯 Keuntungan Dual-Mode System

| Aspek | Keuntungan |
|-------|-----------|
| **Transparansi** | Kedua nilai (auto & manual) tersimpan untuk audit trail |
| **Fleksibilitas** | Owner bisa override saat ada kontrak/negosiasi khusus |
| **Real-time Tracking** | Harga pasar tetap dipantau meskipun menggunakan manual |
| **Easy Rollback** | Bisa kembali ke auto dengan satu klik tanpa kehilangan data |
| **Compliance** | Semua perubahan manual tercatat dengan alasan dan timestamp |
...
```json
{
  ...
  "min_price": 45000,
  "max_price": 60000,
  "avg_price": 50265,           // Harga otomatis (dari transaksi)
  "avg_price_auto": 50265,      // Sama dengan avg_price
  "avg_price_manual": 55000,    // Harga manual (override)
  "is_avg_manual": true,        // Flag: sedang menggunakan manual
  "effective_avg": 55000,       // Yang digunakan sistem (prioritas manual)
  "formatted": {
    "min": "Rp 45.000",
    "max": "Rp 60.000",
    "avg": "Rp 55.000"          // Formatted dari effective_avg
  }
}
```
...
| `POST`| `/price-index/{id}/update-avg-manual` | `price-index.update-avg-manual` | Set/update/hapus AVG manual |
...
Untuk efisiensi dan performa, sistem menggunakan **Incremental Moving Average** untuk menghitung `avg_price` secara real-time tanpa perlu query ulang seluruh histori transaksi.
...
Di mana:
- `old_avg` = Rata-rata harga sebelumnya
- `n` = Jumlah transaksi sebelumnya
- `new_price` = Harga dari transaksi baru yang approved
- `new_avg` = Rata-rata harga yang baru

**Contoh Perhitungan:**
```php
// Data existing
$old_avg = 50000;  // Rp 50,000
$n = 10;           // Dari 10 transaksi

// Transaksi baru approved
$new_price = 52000; // Rp 52,000

// Hitung new average
$new_avg = (($old_avg * $n) + $new_price) / ($n + 1);
$new_avg = ((50000 * 10) + 52000) / 11;
$new_avg = 552000 / 11;
$new_avg = 50182; // Rp 50,182
```

**Keuntungan:**
- ⚡ **Performa Tinggi**: Tidak perlu query ribuan transaksi setiap kali ada approval
- 🎯 **Akurat**: Hasil sama dengan menghitung ulang dari awal
- 🔄 **Real-time**: Update langsung saat transaksi di-approve
- 💾 **Efisien**: Hanya butuh 3 nilai (old_avg, n, new_price)
...
public function processApprovedItem(string $itemName, float $price, ?string $category = null): ?PriceIndex
{
    ...
    if ($price >= $pi->min_price && $price <= $pi->max_price) {
        ...
        $pi->update([
            ...
            // avg_price_manual TIDAK disentuh
        ]);
    }
...
**Catatan Penting:**
- ✅ Hanya transaksi dengan harga **dalam range [Min, Max]** yang masuk perhitungan
- ✅ `avg_price_manual` **TIDAK PERNAH** disentuh oleh proses otomatis
- ✅ Sistem tetap menghitung `avg_price` meskipun ada `avg_price_manual` (untuk tracking pasar)
