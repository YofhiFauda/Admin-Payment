...
- **Telegram**: Real-time untuk critical anomalies (>50%) & Daily summary untuk minor anomalies
...
Nodes:
  ...
  3. Switch (Route by severity)
     - Critical → Telegram only
     - Medium → Telegram only
     - Low → Telegram only
  4. Telegram (Send message)
  5. HTTP Request (Callback to Laravel: notification_sent)
```
...
Nodes:
  ...
  4. Telegram (Send summary to Owner)
```
