            ...
            <div class="flex items-center gap-0.5 sm:gap-1 md:gap-2 min-w-max border-b border-gray-100">
                @php
                    $tabs = [
                        ...
                        'approved'  => ['label' => 'Menunggu Owner', 'count' => $stats['approved'] ?? 0],
...
async function loadServerSideData() {
        ...
        function generateDesktopRow(t, rowNum = '') {
            ...
            const statusLabel = {
                ...
                completed: 'Selesai',
                rejected:  'Ditolak',
                waiting_payment: t.type === 'gudang' ? 'Belum dibayar' : 'Menunggu Pembayaran',
                flagged:   'Flagged',
                ...
                html = `
                    <div class="flex items-center gap-1 ml-1">
                        <button type="button" onclick="performStatusAction(${t.id}, 'approved', this)"
                            class="p-1.5 rounded-lg bg-green-50 text-green-600 hover:bg-green-600 hover:text-white border border-green-200 active:scale-90 transition-all outline-none">
                            <i data-lucide="check" class="w-3 h-3"></i>
                        </button>
                        <button type="button" onclick="openRejectModal(${t.id}, '${t.invoice_number}')"
                            class="p-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-600 hover:text-white border border-red-200 active:scale-90 transition-all outline-none">
        ...
        function generateMobileActions(t) {
            if (typeof canManage === 'undefined' || !canManage || t.status !== 'pending') return '';
            return `
                <button type="button" onclick="performStatusAction(${t.id}, 'approved', this)"
                    class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white font-semibold text-[11px] active:scale-95 transition-all border border-emerald-200 outline-none">
        ...
        return {
            ...
            goToPage: goToPage,
            refresh: loadData,
            getAll: () => mode === 'client' ? allTransactions : [],
