...
<div class="p-4 md:p-6">
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-900">Tambah Piutang Usaha</h1>
                <p class="text-sm text-slate-500">Isi detail pencairan piutang dengan lengkap</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('pengeluaran-lain.piutang-usaha.store') }}" enctype="multipart/form-data" 
            ...
            {{-- Bukti --}}
            <div>
                <label class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Bukti Transfer (JPG/PNG max 2MB)</label>
                <div class="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-all" onclick="document.getElementById('bukti_transfer').click()">
                    <div id="preview-container" class="hidden mb-3"><img id="preview-img" class="max-h-48 mx-auto rounded-xl shadow"></div>
                    <p class="text-sm font-semibold text-slate-500">Pilih Bukti Transfer</p>
                </div>
            ...
            {{-- Cabang --}}
            <div>
                <label class="block text-xs font-black text-slate-500 uppercase mb-2">Unit/Cabang Pemberi <span class="text-rose-500">*</span></label>
                <select name="branch_id" required class="w-full px-4 py-3 border rounded-xl font-semibold bg-white">
                    <option value="">— Pilih Cabang —</option>
                    @foreach($branches as $branch) <option value="{{ $branch->id }}" {{ old('branch_id') == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option> @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3" placeholder="..." class="w-full px-4 py-3 border rounded-xl">{{ old('keterangan') }}</textarea>
            </div>

            <div class="flex items-center gap-3 pt-2">
                <a href="{{ route('pengeluaran-lain.piutang-usaha.index') }}" class="flex-1 px-6 py-3 border rounded-xl text-center">Batal</a>
                <button type="submit" class="flex-[2] px-6 py-3 bg-slate-900 text-white font-bold rounded-xl shadow-lg">Simpan Piutang Usaha</button>
            </div>
        </form>
    </div>
</div>
