        ...
        <div class="p-6 sm:p-8 border-b border-slate-100 bg-slate-50/30">
            <div class="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-[1.25rem] bg-linear-to-br from-sky-600 to-sky-500 flex items-center justify-center shadow-lg shadow-sky-500/20 shrink-0">
                    <i data-lucide="user-cog" class="w-7 h-7 sm:w-8 sm:h-8 text-white"></i>
                </div>
                <div>
                    <h3 class="text-xl sm:text-2xl font-black text-slate-900 leading-tight">Edit Akun Pengguna</h3>
                    <p class="text-[11px] sm:text-xs text-slate-500 font-bold uppercase tracking-[0.2em] mt-1.5">Perbarui informasi profil <strong>{{ $user->name }}</strong></p>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route('users.update', $user->id) }}" class="p-6 sm:p-10 space-y-8" id="editUserForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {{-- Password (opsional) --}}
                <div class="space-y-3 group">
                    <label for="password" class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] ml-1 group-focus-within:text-sky-600 transition-colors">Password Baru <span class="text-slate-300 lowercase font-medium">(opsional)</span></label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-4.5 flex items-center pointer-events-none text-slate-300 group-focus-within:text-sky-500 transition-colors">
            ...
            {{-- Role --}}
            <div class="space-y-3 group">
                <label for="role" class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] ml-1 group-focus-within:text-sky-600 transition-colors">Role / Peran</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4.5 flex items-center pointer-events-none text-slate-300 group-focus-within:text-sky-500 transition-colors">
