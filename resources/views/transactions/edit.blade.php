    ...
    <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('transactions.index') }}"
                class="p-2.5 bg-slate-100 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all">
                <i data-lucide="arrow-left" class="w-4 h-4"></i>
            </a>
            <div>
                <h2 class="text-base md:text-lg font-black text-slate-800">Edit Transaksi</h2>
        ...
        <form method="POST" action="{{ route('transactions.update', $transaction->id) }}" id="transaction-form"
            ...
            <div class="space-y-3 md:space-y-4 lg:space-y-5">
                ...
                @if($transaction->file_path)
                    <div class="bg-white/80 backdrop-blur-sm p-4 rounded-xl shadow-md border border-gray-100">
                        <p class="text-xs font-bold text-slate-400 uppercase mb-2">Nota</p>
                        <div class="bg-slate-50 rounded-lg overflow-hidden text-center p-4">
                            <img src="{{ route('transactions.image', $transaction->id) }}"
                                class="w-full max-h-60 object-contain mx-auto"
                ...
                {{-- Main Info Card --}}
                <div class="bg-white/80 backdrop-blur-sm p-3 md:p-4 lg:p-6 rounded-lg md:rounded-xl lg:rounded-2xl shadow-md hover:shadow-lg border border-gray-100 border-b-4 border-b-blue-600 transition-all duration-300">
                ...
                {{-- Branch Allocation --}}
                <div class="bg-white/80 backdrop-blur-sm p-3 md:p-4 lg:p-6 rounded-lg md:rounded-xl lg:rounded-2xl shadow-md hover:shadow-lg border border-gray-100 transition-all duration-300">

                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 md:gap-3 mb-3 md:mb-4 lg:mb-5">
                        <h3 class="text-xs md:text-sm lg:text-base font-bold flex items-center gap-1.5 md:gap-2 text-slate-800">
                        ...
                        <div class="flex bg-slate-100 p-0.5 md:p-1 rounded-md md:rounded-lg w-full sm:w-auto">
                            <button type="button" data-mode="equal" class="alloc-mode-btn flex-1 sm:flex-none px-2.5 md:px-3 lg:px-4 py-1.5 md:py-2 rounded text-xs md:text-sm lg:text-[10px] font-bold transition-all text-slate-400">RATA</button>
                            <button type="button" data-mode="percent" class="alloc-mode-btn flex-1 sm:flex-none px-2.5 md:px-3 lg:px-4 py-1.5 md:py-2 rounded text-xs md:text-sm lg:text-[10px] font-bold transition-all bg-white shadow text-blue-600">PERSEN</button>
                            <button type="button" data-mode="manual" class="alloc-mode-btn flex-1 sm:flex-none px-2.5 md:px-3 lg:px-4 py-1.5 md:py-2 rounded text-xs md:text-sm lg:text-[10px] font-bold transition-all text-slate-400">MANUAL</button>
                        </div>
                    </div>

                    <div id="branches-container" class="space-y-2 md:space-y-3">
                        @foreach($transaction->branches as $i => $branch)
                            <div class="branch-row flex flex-col sm:flex-row gap-2 md:gap-3 items-end bg-slate-50 p-2.5 md:p-3 lg:p-4 rounded-lg md:rounded-xl border border-slate-100"
                                ...
                                <div class="flex-1 w-full">
                                    ...
                                    <div class="relative">
                                        <select name="branches[{{ $i }}][branch_id]"
                                ...
                                <div class="w-full sm:w-32 md:w-36">
                                    <label class="alloc-label block text-xs md:text-sm font-bold text-slate-400 uppercase mb-1 md:mb-1.5 tracking-wider">Porsi (%)</label>
                                    <div class="relative">
                                        <input type="number" step="0.01"
                                            ...
                                            value="{{ $branch->pivot->allocation_percent }}" />
                                        <span class="alloc-suffix absolute right-2.5 md:right-3 top-1/2 -translate-y-1/2 font-bold text-slate-300 text-xs md:text-sm pointer-events-none">%</span>
                                    </div>
                                </div>
                                <input type="hidden" name="branches[{{ $i }}][allocation_percent]" class="alloc-percent-hidden"
                                    value="{{ $branch->pivot->allocation_percent }}" />
                                <input type="hidden" name="branches[{{ $i }}][allocation_amount]" class="alloc-amount"
                                    value="{{ $branch->pivot->allocation_amount }}" />
                                @if($i > 0)
                                    <button type="button"
            ...
            {{-- Summary --}}
            <div>
                <div class="bg-slate-900 p-4 md:p-5 lg:p-6 rounded-lg md:rounded-xl lg:rounded-2xl text-white shadow-xl lg:sticky lg:top-24 border border-white/5">
                    <h4 class="text-xs md:text-sm font-bold text-blue-400 mb-4 md:mb-5 uppercase tracking-widest">
                        Billing Summary
                    </h4>
                    <div class="space-y-3 md:space-y-4 mb-5 md:mb-6">
                        <div class="flex justify-between items-end border-b border-white/10 pb-3 md:pb-4">
                            ...
                            <span id="summary-total" class="font-bold text-base md:text-lg lg:text-xl text-white tracking-tight">Rp 0</span>
                        </div>
                        <div class="flex justify-between items-center border-b border-white/10 pb-3 md:pb-4">
                            ...
                            <div id="alloc-badge" class="px-2.5 md:px-3 py-1 md:py-1.5 rounded-full flex items-center gap-1.5 md:gap-2 font-bold text-xs md:text-sm uppercase tracking-wider bg-red-500/20 text-red-400">
                        ...
                        <div class="pt-3 border-t border-white/10 hidden lg:block">
                            <p class="text-[9px] font-bold text-slate-500 uppercase tracking-wider mb-3">Rincian Distribusi</p>
                            <div id="summary-branches" class="space-y-2 text-xs"></div>
                        </div>
                    </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                document.getElementById('add-branch-btn').addEventListener('click', function () {
                    ...
                    const opts = branchesJson.map(b => `<option value="${b.id}">${b.name}</option>`).join('');

                    const row = document.createElement('div');
                    row.className = 'branch-row flex flex-col sm:flex-row gap-2 md:gap-3 items-end bg-slate-50 p-2.5 md:p-3 lg:p-4 rounded-lg md:rounded-xl border border-slate-100';
                    ...
                    row.innerHTML = `
                        <div class="flex-1 w-full">
                            <label class="block text-xs md:text-sm font-bold text-slate-400 uppercase mb-1 md:mb-1.5 tracking-wider">Cabang</label>
                            <div class="relative">
                                <select name="branches[${idx}][branch_id]"
                                    class="branch-select w-full appearance-none border border-slate-200 rounded-md md:rounded-lg p-2 md:p-2.5 text-xs md:text-sm font-medium bg-white pr-7 md:pr-8 outline-none focus:ring-2 focus:ring-blue-100">
                                    ${opts}
                                </select>
                        ...
                        <div class="w-full sm:w-32 md:w-36">
                            <label class="alloc-label block text-xs md:text-sm font-bold text-slate-400 uppercase mb-1 md:mb-1.5 tracking-wider">
                                ${allocMode === 'manual' ? 'Nominal' : 'Porsi (%)'}
                            </label>
                            <div class="relative">
                                <input type="number" step="0.01" value="0"
                                    class="alloc-percent w-full border border-slate-200 rounded-md md:rounded-lg p-2 md:p-2.5 pr-7 md:pr-8 text-xs md:text-sm font-bold bg-white outline-none focus:ring-2 focus:ring-blue-100" />
                                <span class="alloc-suffix absolute right-2.5 md:right-3 top-1/2 -translate-y-1/2 font-bold text-slate-300 text-xs md:text-sm pointer-events-none">
                                    ${allocMode === 'manual' ? '' : '%'}
                                </span>
                            </div>
                        </div>
                ...
                function enforceUniqueBranches() {
                    ...
                    selects.forEach(select => {
                        if (select.value) usedBranches.push(select.value);
                    });
                    selects.forEach(select => {
                        const currentValue = select.value;
                        select.querySelectorAll('option').forEach(option => {
                            if (!option.value) return;
                            option.disabled = usedBranches.includes(option.value) && option.value !== currentValue;
                        });
                    });
                }
